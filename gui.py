...
def go_to_omop_errors():
    ...
    standard = session.get("standard")
    if standard == "fhir":
        return render_template('dashboard.html', graphJSON=pgraph)
    else:
        return redirect(url_for("errors_omop"))
...
def fire():
    ...
    pgraph = quality_checks_fhir.completeness(pdf)
    sgraph = quality_checks_fhir.completeness(sdf)
    cgraph = quality_checks_fhir.completeness(cdf)
